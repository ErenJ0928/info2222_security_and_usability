...
def run_server():
    ...
    run(host=host, port=port, debug=debug, server='gunicorn', certfile='hellfish.test.crt', keyfile='hellfish.test.key')
...
def manage_db():
    ...
    user_data_base = sql.SQLDatabase("user_database.db")
    user_data_base.database_setup()
    user_data_base.cur.close()
