...
class View():
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(list(kwargs.values())) != 0:
            un = list(kwargs.values())[0]
            #print(un)
            if un in list(request.cookies.keys()):
                # print(1)
                # print(list(kwargs.values()))
                #response.set_cookie("visit", un)
                return self.load_and_render_login(*args, **kwargs)
            elif un == 1:
                return self.load_and_render_admin(*args, **kwargs)
            else:
                # print(2)
                # print(list(kwargs.values()))
    ...
    def load_and_render_login(self, filename, header="login_header", tailer="tailer", **kwargs):
        '''
            Loads and renders templates
    ...
    def load_and_render_admin(self, filename, header="admin_header", tailer="tailer", **kwargs):
        '''
            Loads and renders templates
